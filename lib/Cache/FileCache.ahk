class FileCache extends CacheBase {
    ...
    __New(tmpDir, stateObj, parentDir, cacheDirName := "") {
        cachePath := parentDir

        if (cacheDirName != "") {
            cachePath := cachePath . "\" . cacheDirName
        }
        ...
        super.__New(tmpDir, stateObj)
    }

    ConvertUrlToPathName(url) {
        folderName := StrLower(url)
        folderName := StrReplace(folderName, "http://")
        folderName := StrReplace(folderName, "https://")
        folderName := StrReplace(folderName, "\", "--")
        folderName := StrReplace(folderName, "/", "--")
        folderName := StrReplace(folderName, ":", "--")
        return folderName
    }
